...
export const FileUploadDropZone = ({
    ...
    return (
        <div
            ...
            onDrop={handleDrop}
            className={cx(
                "relative flex flex-col items-center gap-3 rounded-xl bg-uui-primary px-6 py-4 text-uui-tertiary ring-1 ring-uui-secondary transition duration-100 ease-linear ring-inset",
                isDraggingOver && "ring-2 ring-brand",
                isDisabled && "cursor-not-allowed bg-uui-secondary",
                className,
            )}
        >
...
export const FileListItemProgressBar = ({ name, size, progress, failed, type, fileIconVariant, onDelete, onRetry, className }: FileListItemProps) => {
    ...
    return (
        <motion.li
            layout="position"
            className={cx(
                "relative flex gap-3 rounded-xl bg-uui-primary p-4 ring-1 ring-uui-secondary transition-shadow duration-100 ease-linear ring-inset",
            ...
            <div className="flex min-w-0 flex-1 flex-col items-start">
                <div className="flex w-full max-w-full min-w-0 flex-1">
                    <div className="min-w-0 flex-1">
                        <p className="truncate text-sm font-medium text-fg-uui-secondary">{name}</p>

                        <div className="mt-0.5 flex items-center gap-2">
                            <p className="truncate text-sm whitespace-nowrap text-uui-tertiary">{getReadableFileSize(size)}</p>

                            <hr className="h-3 w-px rounded-t-full rounded-b-full border-none bg-border-uui-primary" />
                            ...
                            <div className="flex items-center gap-1">
                                ...
                                {!isComplete && !failed && <p className="text-sm font-medium text-uui-quaternary">Uploading...</p>}
...
export const FileListItemProgressFill = ({ name, size, progress, failed, type, fileIconVariant, onDelete, onRetry, className }: FileListItemProps) => {
    ...
    return (
        <motion.li layout="position" className={cx("relative flex gap-3 overflow-hidden rounded-xl bg-uui-primary p-4", className)}>
            {/* Progress fill. */}
            <div
                style={{ transform: `translateX(-${100 - progress}%)` }}
                className={cx("absolute inset-0 size-full bg-uui-secondary transition duration-75 ease-linear", isComplete && "opacity-0")}
                role="progressbar"
            ...
            <div
                className={cx(
                    "absolute inset-0 size-full rounded-[inherit] ring-1 ring-uui-secondary transition duration-100 ease-linear ring-inset",
                    failed && "ring-2 ring-error",
                )}
            />
            ...
            <div className="relative flex min-w-0 flex-1">
                <div className="relative flex min-w-0 flex-1 flex-col items-start">
                    <div className="w-full min-w-0 flex-1">
                        ...
                        <div className="mt-0.5 flex items-center gap-2">
                            <p className="text-sm text-uui-tertiary">{failed ? "Upload failed, please try again" : getReadableFileSize(size)}</p>
                            ...
                            {!failed && (
                                <>
                                    ...
                                    <div className="flex items-center gap-1">
                                        ...
                                        <p className="text-sm text-uui-tertiary">{progress}%</p>
                                    </div>
                                </>
                            )}
                        </div>
